<?php
    class Cataloguing extends DataBoundObject {
        ...
        public function validate() {
            ...
            else if (!preg_match('|^[!\-\+\.\&\'\/0-9a-zA-Z\xE0\xE8\xE9\xF9\xF2\xEC\x27, ]+$|', $this->Titolo))
            {
                $this->errors['Titolo'] = 'Hai inserito un TITOLO non corretto.';
            }
            ...
            //Titolo originale
            if(!empty($this->Titolo_orig))
            {
                if (!preg_match('|^[\-\.\&\'\/0-9a-zA-Z\xE0\xE8\xE9\xF9\xF2\xEC\x27, ]+$|', $this->Titolo_orig))
                {
                    $this->errors['Titolo_orig'] = 'Hai inserito un TITOLO ORIGINALE non corretto.';
                }
            }
